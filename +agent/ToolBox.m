classdef ToolBox < handle
    ...
    methods
        ...
        function register(obj)
            ...
            % MATLAB tools
            obj.registerTool('run_code', @tools.matlab.run_code_or_file, ...
                'Execute arbitrary MATLAB code and return results', ...
                struct('input', 'string', 'isFile', 'logical'));
            
            obj.registerTool('check_code_lint', @tools.matlab.check_code_lint, ...
                'Check MATLAB code for linting issues', ...
                struct('codeStr', 'string'));
                
            obj.registerTool('open_file', @tools.matlab.open_or_create_file, ...
                'Open or create a file in MATLAB editor', ...
                struct('filePath', 'string', 'content', 'string'));
                
            obj.registerTool('read_file', @tools.matlab.read_file_content, ...
                'Read content from a file', ...
                struct('filePath', 'string'));
                
            obj.registerTool('write_file', @tools.matlab.write_file_contents, ...
                'Write content to a file', ...
                struct('filePath', 'string', 'content', 'string'));
                
            obj.registerTool('get_workspace_var', @tools.matlab.get_workspace_var, ...
                'Get a variable from MATLAB workspace', ...
                struct('varName', 'string'));
                
            obj.registerTool('set_workspace_var', @tools.matlab.set_workspace_var, ...
                'Set a variable in MATLAB workspace', ...
                struct('varName', 'string', 'value', 'any'));
                
            obj.registerTool('run_tests', @tools.matlab.run_unit_tests, ...
                'Run MATLAB unit tests', ...
                struct('testPath', 'string'));
                
            obj.registerTool('commit_changes', @tools.matlab.commit_git_repo, ...
                'Commit changes to Git repository', ...
                struct('message', 'string', 'repoPath', 'string'));
            
            % Simulink tools
            obj.registerTool('new_model', @tools.simulink.create_new_model, ...
                'Create a new Simulink model', ...
                struct('modelName', 'string'));
                
            obj.registerTool('open_model', @tools.simulink.open_existing_model, ...
                'Open an existing Simulink model', ...
                struct('modelName', 'string'));
                
            obj.registerTool('close_model', @tools.simulink.close_current_model, ...
                'Close the current Simulink model', ...
                struct('modelName', 'string', 'saveChanges', 'logical'));
                
            obj.registerTool('save_model', @tools.simulink.save_current_model, ...
                'Save the current Simulink model', ...
                struct('modelName', 'string'));
                
            obj.registerTool('add_block', @tools.simulink.insert_library_block, ...
                'Add a block from library to model', ...
                struct('modelName', 'string', 'blockType', 'string', 'position', 'array'));
                
            obj.registerTool('remove_block', @tools.simulink.remove_block, ...
                'Remove a block from the model', ...
                struct('modelName', 'string', 'blockPath', 'string'));
                
            obj.registerTool('connect_blocks', @tools.simulink.connect_block_ports, ...
                'Connect two blocks in the model', ...
                struct('modelName', 'string', 'sourceBlockPath', 'string', 'destBlockPath', 'string'));
                
            obj.registerTool('disconnect_blocks', @tools.simulink.disconnect_block_ports, ...
                'Disconnect blocks in the model', ...
                struct('modelName', 'string', 'blockPath', 'string', 'portNumber', 'double'));
                
            obj.registerTool('get_block_params', @tools.simulink.get_block_params, ...
                'Get parameters of a Simulink block', ...
                struct('modelName', 'string', 'blockPath', 'string'));
                
            obj.registerTool('set_block_params', @tools.simulink.set_block_params, ...
                'Set parameters of a Simulink block', ...
                struct('modelName', 'string', 'blockPath', 'string', 'params', 'struct'));
                
            obj.registerTool('auto_layout', @tools.simulink.auto_layout, ...
                'Automatically arrange blocks in the model for better layout', ...
                struct('modelName', 'string'));
                
            obj.registerTool('simulate_model', @tools.simulink.simulate_model, ...
                'Simulate the model and return results', ...
                struct('modelName', 'string', 'simTime', 'double'));
                
            % General tools
            obj.registerTool('doc_search', @tools.general.doc_search, ...
